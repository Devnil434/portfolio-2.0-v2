...
const Project = ({ index, project, onMouseEnter }: Props) => {
    ...
    return (
        <TransitionLink
            ...
            className="project-item group leading-none py-10 px-6 mb-4 md:border-b-0 mac-border glass rounded-2xl md:group-hover/projects:opacity-30 md:hover:!opacity-100 transition-all hover:bg-primary/[0.03] relative overflow-hidden block"
        ...
        >
            ...
            <div className="md:hidden">
                <Image
                    ...
                    width="800"
                    height="1200"
                    className="w-full h-auto object-contain mb-6 rounded-xl glow-primary mac-border bg-white/[0.02]"
                    loading="lazy"
                />
            </div>

            <div className="flex gap-2 md:gap-5 relative z-[1]">
                <div className="font-anton text-primary/40 group-hover:text-primary transition-colors">
                    _{(index + 1).toString().padStart(2, '0')}.
                </div>
                <div className="">
                    <h4 className="text-4xl xs:text-6xl flex gap-4 font-anton transition-all duration-700 bg-gradient-to-r from-primary to-foreground from-[50%] to-[50%] bg-[length:200%] bg-right bg-clip-text text-transparent group-hover:bg-left group-hover:neon-text">
                        {project.title}
                        <span className="text-primary opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-2">
                            <svg
                    ...
                    <div className="mt-4 flex flex-wrap gap-3 text-muted-foreground/60 text-xs uppercase tracking-widest font-medium">
                        {project.techStack
                            .slice(0, 3)
                            .map((tech, idx, stackArr) => (
                                ...
                                >
                                    ...
                                    {idx !== stackArr.length - 1 && (
                                        <span className="inline-block size-1 rounded-full bg-primary/30"></span>
                                    )}
                                </div>
                            ))}
                    </div>
